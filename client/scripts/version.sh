...
tags=$(git -C "$REPO" tag --points-at HEAD --list 'client/v*')
tag_count=$(printf '%s\n' "$tags" | sed '/^$/d' | wc -l | tr -d ' ')
if [ "$tag_count" -gt 1 ]; then
  echo "multiple client release tags point at HEAD" >&2
  exit 1
fi
...
if [ "$tag_count" -eq 1 ] && [ -z "$dirty" ]; then
  version=${tags#client/v}
  printf '%s\n' "$version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$' || {
    echo "invalid client release tag: $tags" >&2
    exit 1
  }
  echo "$version"
  exit 0
